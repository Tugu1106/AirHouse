...
export async function findBranchByName(name: string): Promise<Branch> {
  const client = getServiceClient();
  const { data, error } = await client.from('branches').select('*').ilike('name', name.trim());
  if (error) throw new Error(error.message);
...
export async function createBranch(name: string): Promise<Branch> {
  const client = getServiceClient();
  const { data, error } = await client.from('branches').insert({ name }).select().single();
  if (error) throw new Error(error.message);
  return data as Branch;
}
...
export async function setBranchAsHq(id: UUID): Promise<void> {
  const client = getServiceClient();
  const { error: clearErr } = await client.from('branches').update({ is_hq: false }).neq('id', id);
  if (clearErr) throw new Error(clearErr.message);
  const { error: setErr } = await client.from('branches').update({ is_hq: true }).eq('id', id);
...
export async function branchItemCounts(): Promise<Record<UUID, number>> {
  const client = getServiceClient();
  const { data, error } = await client.from('items').select('branch_id').is('deleted_at', null);
  if (error) throw new Error(error.message);
