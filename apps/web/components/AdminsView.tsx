    ...
    <main className="mx-auto max-w-3xl space-y-6 px-6 py-6">
      <div>
        ...
        <p className="text-sm text-slate-400">
          Give each person their own admin login so the activity log shows who did what.
        </p>
      </div>
            ...
            {admins.map((a) => (
              <tr key={a.id}>
                <td className="px-4 py-3 font-medium text-slate-200">
                  {a.email}
                  {a.id === currentUserId && (
                    <span className="ml-2 rounded-full bg-brand/15 px-2 py-0.5 text-xs text-brand">you</span>
                  )}
                </td>
